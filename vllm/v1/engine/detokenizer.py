...
class FastIncrementalDetokenizer(BaseIncrementalDetokenizer):
    def __init__(self, tokenizer: PreTrainedTokenizerFast, request: EngineCoreRequest):
        ...
        # DISABLED: No longer using DecodeStream for incremental decoding
        # self.stream = DecodeStream(skip_special_tokens=self.skip_special_tokens)

        self.tokenizer: Tokenizer = tokenizer._tokenizer
        self.full_tokenizer = tokenizer  # Store the full tokenizer for batch decoding

        # DISABLED: No longer priming the stream
        ...
        # Store the previous decoded text to compute deltas
        self.previous_text = ""
        self.prompt_len = len(request.prompt_token_ids or [])

        # DISABLED: No longer handling special tokens via stream
    ...
    def decode_next(self, next_token_id: int) -> str:
        """Decode by decoding all tokens from scratch (non-incremental).
        
        This avoids issues with custom tokenizers that have problems with
        incremental detokenization.
        """
        # Get only the output tokens (excluding prompt)
        output_token_ids = self.token_ids[self.prompt_len:] if self.prompt_len > 0 else self.token_ids
        
        # Decode all output tokens from scratch
        try:
            current_text = self.full_tokenizer.decode(
                output_token_ids,
                skip_special_tokens=self.skip_special_tokens
            )
        except Exception as e:
            logger.exception("Failed to decode token ids: %r", output_token_ids)
            current_text = self.previous_text
        
        # Compute the delta (new text since last decode)
        if current_text.startswith(self.previous_text):
            delta = current_text[len(self.previous_text):]
        else:
            # If current text doesn't start with previous text, 
            # the tokenizer may have changed the decoding - return the full diff
            delta = current_text
        
        self.previous_text = current_text
        return delta

    # DISABLED: No longer needed with non-incremental decoding
...
class SlowIncrementalDetokenizer(BaseIncrementalDetokenizer):
    def __init__(self, tokenizer: AnyTokenizer, request: EngineCoreRequest):
        ...
        # DISABLED: No longer using incremental detokenization metadata
    ...
    def decode_next(self, next_token_id: int) -> str:
        ...
        incremental detokenization.
        """
        # DISABLED: No longer using incremental detokenization
